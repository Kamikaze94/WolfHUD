...
local BRIEFING_PROFILE_MENU = false
...
if RequiredScript == "lib/managers/menumanager" then    
    ...
    function LobbyOptionInitiator:modify_node(node, ...)
        ...
        if briefing_nodes and briefing_nodes.kit and not briefing_nodes[PROFILE_MENU_ID] then
            create_profile_menu_node(briefing_nodes, PROFILE_MENU_ID)
            BRIEFING_PROFILE_MENU = true
        end
    ...
    function ProfileSwitchInitiator:add_back_button(node)
    ...
    function MenuCallbackHandler:profile_menu_switch(item)
        ...
        if not BRIEFING_PROFILE_MENU then
            self:refresh_node()
        else
...
if RequiredScript == "lib/managers/menu/missionbriefinggui" then
    ...
    function MissionBriefingGui:special_btn_pressed(button, ...)
        local active_menu = managers.menu:active_menu()
        local controller = active_menu and active_menu.input:get_controller_class()
        if self._enabled and controller and button == Idstring("menu_change_profile_right") and controller:get_input_pressed("menu_change_profile_left") then
            managers.menu:open_node(PROFILE_MENU_ID, {})
        end
...
elseif RequiredScript == "lib/managers/menu/playerinventorygui" then
    ...
    function PlayerInventoryGui:special_btn_pressed(button, ...)
        local active_menu = managers.menu:active_menu()
        local controller = active_menu and active_menu.input:get_controller_class()
        if controller and button == Idstring("menu_change_profile_right") and controller:get_input_pressed("menu_change_profile_left") then
            managers.menu:open_node(PROFILE_MENU_ID, {})
        end
